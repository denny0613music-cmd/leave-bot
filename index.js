...
function buildSystemPrompt() {
  return [
    ...
    "在聊天時，可以適度開玩笑、亂猜、拋出選項，但必須讓人知道你是在『陪聊』，不是在陳述事實。",
...
async function askGeminiChat({ authorName, userText, userId }) {
  ...
  // ✅ 聊天模式：明確禁止搜尋/附來源/貼連結
    const chatMode = [
    "你現在在進行日常聊天。",
    "規則：不要查資料、不要提供來源、不要列連結。",
    "可以用聊天的方式接話、開玩笑、亂猜，但必須明確讓人知道你是在陪聊或猜測，而不是在陳述事實。",
    "如果使用者要你猜、通靈、隨便說，可以用輕鬆語氣回應，例如給 2~3 個可能選項。",
    "保持親切秘書的人格設定，用繁體中文，語氣自然。",
    "避免過度正式或像客服一樣的拒絕式回答。",
  ].join("
");
